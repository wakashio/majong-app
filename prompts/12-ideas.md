...
### 2026-01-12 Aboutページをアップデートしたい

### [議論完了] Aboutページをアップデートしたい

- **内容**: Aboutページ（`frontend/src/views/AboutView.vue`）の内容を更新する。現在のページには基本的なアプリ説明、主な機能、使い方、バージョン情報が含まれているが、より詳細な情報や最新の機能情報を追加する必要がある。具体的なアップデート内容は要確認。
- **提案者**: ユーザー
- **優先度**: 中
- **関連機能**: AboutView.vue、UI/UX、ドキュメント
- **備考**: 現在のAboutページにはバージョン情報（1.0.0）、更新日（2025-12-30）が固定値で設定されている。セッション機能など、追加された機能の説明が含まれていない可能性がある。設計ドキュメント（`design/screen/about-screen.md`）も存在するため、設計と実装の整合性を確認する必要がある。
- **議論内容**: 現在の実装と設計ドキュメント、実際の機能を比較し、差異を洗い出した。セッション機能が実装されているが記載されていない、使い方の手順が古い、バージョン情報が固定値などの問題を確認。バージョン管理はAboutページ独自で管理する方針で決定。セッション機能を主な機能リストに追加し、使い方の手順を最新フローに更新する方針で決定。
- **ステータス**: 議論完了 → 議事録に移行済み
- **議事録参照**: `11-meeting-notes.md` (2026-01-12 Aboutページのアップデート)
...
- **内容**: 局終了時のテストケースを追加する。現在、`PUT /api/rounds/:id/end`のテストケースは一部存在しているが、より網羅的なテストケースを追加して、局終了処理の動作を確実に検証できるようにする。特に、様々なシナリオ（ツモ、ロン、流局、特殊流局、ダブロン、トリロン、連荘、本場計算、積み棒計算など）のテストケースを追加し、点数計算の正確性を検証する。
- **提案者**: ユーザー
- **優先度**: 中
- **関連機能**: 局終了処理、テスト、点数計算、本場計算、積み棒計算、連荘判定、`roundService.ts`、`roundController.ts`、`tests/integration/rounds.test.ts`、`scoreCalculationService.ts`
- **備考**: 既存のテストファイル（`backend/tests/integration/rounds.test.ts`）には一部のテストケースが存在するが、より網羅的なテストケースを追加することで、局終了処理の動作を確実に検証できるようになる。特に、エッジケースや複雑なシナリオ（ダブロン時の積み棒分配、本場計算、連荘判定など）のテストケースを追加する必要がある。

### 2026-01-12 「アクションを追加」で鳴いた参加者を選んだ時に対象参加者で同じ人を選べないようにしたい

### [議論完了] 「アクションを追加」で鳴いた参加者を選んだ時に対象参加者で同じ人を選べないようにしたい

- **内容**: 鳴き記録の追加時に、鳴いた参加者と対象参加者が同じ人を選べないようにするバリデーションを追加したい。現在の実装では、鳴いた参加者と対象参加者で同じ人を選択できてしまう可能性があるが、麻雀のルール上、自分自身を対象に鳴くことはできないため、このような選択を防ぐ必要がある。
- **提案者**: ユーザー
- **優先度**: 中
- **関連機能**: 局管理、鳴き記録、UI/UX、バリデーション、`RoundManageView.vue`、鳴き追加ダイアログ、`ActionAddDialog.vue`（コンポーネント分割後）
- **備考**: 鳴き記録追加ダイアログで、鳴いた参加者を選択した際に、対象参加者の選択肢からその参加者を除外する、または選択できないようにする必要がある。ポン、チー、大明槓の場合は対象参加者が存在するが、暗槓の場合は対象参加者が不要であるため、暗槓の場合は対象参加者の選択を無効化または非表示にする必要がある。
- **議論内容**: 現状の問題点を確認。`ActionAddDialog.vue`の115-123行目で、対象参加者の選択肢に`props.playerOptions`をそのまま使用しており、鳴いた参加者（`props.playerId`）も選択肢に含まれている。実装方針: `ActionAddDialog.vue`にcomputedプロパティを追加し、`playerOptions`から`playerId`を除外した選択肢を返す。対象参加者の`PlayerSelectButton`の`:items`にそのcomputedプロパティを指定する。暗槓の場合は既に`v-if="props.nakiType !== NakiType.ANKAN"`で非表示になっているため、追加対応は不要。実装箇所: `frontend/src/components/ActionAddDialog.vue`。
- **ステータス**: 議論完了 → 議事録に移行待ち
