...
### 2026-01-12 Aboutページのアップデート

### [未反映] Aboutページのアップデート

- **元のアイデア**: `12-ideas.md` (2026-01-12 Aboutページをアップデートしたい)
- **議論内容**:
  - 現在の実装と設計ドキュメント、実際の機能を比較し、差異を洗い出した
  - 主な差異:
    - セッション機能が実装されているが、Aboutページに記載されていない
    - バージョン情報が固定値（1.0.0、2025-12-30）で、更新日が古い
    - 使い方の手順が古く、セッション機能を含む最新フローが反映されていない
    - アプリ説明にセッション機能の言及がない
  - バージョン管理について議論: `package.json`から取得する設計だったが、Aboutページ独自で管理する方針に決定
- **決定事項**:
  - バージョン管理: Aboutページ独自で管理（`package.json`から取得しない）
  - 更新日: 現在の日付（2026-01-12）に更新
  - セッション機能の説明: 他の機能と同程度の詳細で記載
  - アップデート内容:
    1. アプリ説明にセッション機能を追加
    2. 主な機能リストにセッション管理を追加（アイコン: `mdi-calendar-multiple`）
    3. 使い方の手順をセッション機能を含めた最新フローに更新（6ステップ）
    4. バージョン情報の更新日を2026-01-12に更新
- **実装方針**:
  - `frontend/src/views/AboutView.vue`を更新
  - セッション管理機能を主な機能リストに追加
  - 使い方の手順を「参加者登録 → セッション作成 → 半荘開始 → 局記録 → 局終了 → 半荘終了」の6ステップに更新
  - アプリ説明にセッション機能を追加
  - 更新日を`2026-01-12`に更新
- **反映先**: `frontend/src/views/AboutView.vue`、`design/screen/about-screen.md`（必要に応じて）
- **優先度**: 中

### 2026-01-12 リーチ者がツモ上がりした時の点数計算の不具合とテストケース追加

### [未反映] リーチ者がツモ上がりした時の点数計算の不具合とテストケース追加

- **元のアイデア**: `12-ideas.md` (2026-01-12 局終了時のテストケースを追加したい)
- **議論内容**:
  - リーチ者がツモ上がりした時の点数計算に不具合があることを確認
  - 問題点: リーチ棒を払った後にリーチ棒を得る計算が正しく行われていない可能性がある
  - 現在の実装フロー:
    - ツモ時、フロントエンドで`calculateTsumoScore`により積み棒を含めた点数を計算
    - バックエンドはフロントエンドから送信された点数をそのまま使用（`roundService.ts` 878-894行目）
    - 局終了時にリーチ記録を確認し、リーチ宣言者に-1000点を計算（`roundService.ts` 965-971行目）
    - この-1000点は最終的な点数変動に加算される（998行目）
  - 問題: リーチ者がツモで上がった場合、フロントエンドで計算された点数には積み棒が含まれているが、リーチ記録による-1000点も別途計算されるため、計算が正しく行われていない可能性がある
- **決定事項**:
  - リーチ者がツモで上がった場合のテストケースを追加する
  - 親と子の両方のケースを追加する
  - 追加すべきテストケース:
    1. **親がリーチを宣言してツモで上がった場合**: リーチ棒を払った後にリーチ棒を得る計算が正しく行われているか確認
    2. **子がリーチを宣言してツモで上がった場合**: リーチ棒を払った後にリーチ棒を得る計算が正しく行われているか確認
    3. **複数のリーチ者がいる場合**: 自分がリーチを宣言した分のリーチ棒は相殺されるが、他の人がリーチを宣言した分の積み棒は獲得できることを確認
- **実装方針**:
  - `backend/tests/integration/rounds.test.ts`に以下のテストケースを追加:
    - 親がリーチを宣言してツモで上がった場合のテストケース
    - 子がリーチを宣言してツモで上がった場合のテストケース
    - 複数のリーチ者がいる場合のテストケース（親がリーチを宣言、子1がリーチを宣言、親がツモで上がる）
  - 各テストケースで、リーチ棒を払った後にリーチ棒を得る計算が正しく行われているかを確認
  - 期待される動作:
    - リーチ者がツモで上がった場合、リーチ棒の-1000点と積み棒の+1000点が相殺されるべき
    - 自分がリーチを宣言した分のリーチ棒は相殺されるが、他の人がリーチを宣言した分の積み棒は獲得できる
- **反映先**: `backend/tests/integration/rounds.test.ts` (PUT /api/rounds/:id/end テストセクション)
- **優先度**: 中
- **関連機能**: 局終了処理、点数計算、リーチ記録、積み棒計算、`roundService.ts`、`roundController.ts`、`tests/integration/rounds.test.ts`、`scoreCalculationService.ts`

### 2026-01-12 「アクションを追加」で鳴いた参加者を選んだ時に対象参加者で同じ人を選べないようにしたい

### [反映済み] 「アクションを追加」で鳴いた参加者を選んだ時に対象参加者で同じ人を選べないようにしたい

- **元のアイデア**: `12-ideas.md` (2026-01-12 「アクションを追加」で鳴いた参加者を選んだ時に対象参加者で同じ人を選べないようにしたい)
- **議論内容**:
  - 現状の問題点を確認: `ActionAddDialog.vue`の115-123行目で、対象参加者の選択肢に`props.playerOptions`をそのまま使用しており、鳴いた参加者（`props.playerId`）も選択肢に含まれている
  - 麻雀のルール上、自分自身を対象に鳴くことはできないため、このような選択を防ぐ必要がある
  - 暗槓の場合は既に`v-if="props.nakiType !== NakiType.ANKAN"`で非表示になっているため、追加対応は不要
- **決定事項**:
  - **実装方針**: `ActionAddDialog.vue`にcomputedプロパティを追加し、`playerOptions`から`playerId`を除外した選択肢を返す
  - 対象参加者の`PlayerSelectButton`の`:items`にそのcomputedプロパティを指定する
  - 暗槓の場合は対象参加者の選択を非表示にする（既に実装済み）
- **実装方針**:
  - `ActionAddDialog.vue`に`targetPlayerOptions`というcomputedプロパティを追加
  - `targetPlayerOptions`は`playerOptions`から`playerId`に一致する項目を除外した配列を返す
  - 対象参加者の`PlayerSelectButton`の`:items`に`targetPlayerOptions`を指定
- **反映先**: `design/components/round-manage-components.md` (ActionAddDialogコンポーネントの仕様)
- **優先度**: 中
- **期限**: （未設定）
